...
- Подтипы системных дефектов:
    ...
    г. Ошибки совместимости: Проблемы при работе на разных платформах, устройствах или версиях ОС.
...
df - Показывает информацию о использовании дискового пространства.
```
df -h
```
...
Эти логи содержат информацию об установке, обновлении и удалении пакетов.
    ...
    /var/log/dpkg.log (в Debian/Ubuntu):
...
#### Службы (или демоны) — это процессы, работающие в фоновом режиме. Они часто управляются через systemd (в современных дистрибутивах Linux). Основные команды для работы со службами:
...
Убедитесь, что сервер принимает логи по UDP или TCP. Откройте файл конфигурации:
```
sudo nano /etc/rsyslog.conf
```
5.Раскомментируйте строки:
```
module(load="imudp")
...
sudo systemctl restart rsyslog
```
7.Утилита logger позволяет отправлять сообщения в системный лог из командной строки.
```
logger "Это тестовое сообщение"
```
#### Примеры использования syslog:
1. Просмотр всего файла:
```
cat /var/log/syslog
```
2. Поиск логов, содержащих слово "error":
```
grep "error" /var/log/syslog
```
3. Отслеживание логов в реальном времени:
```
tail -f /var/log/syslog
```
4. Поиск логов за определённую дату (если дата указана в логах):
```
grep "Oct  1" /var/log/syslog
```
5. Поиск логов, связанных с SSH:
```
grep "sshd" /var/log/syslog
```
6. Поиск логов с уровнем "error" за последние 24 часа
```
grep "error" /var/log/syslog | grep "$(date '+%b %d')"
```
7. Поиск логов с уровнем "error" и вывод только времени и сообщения
```
awk '/error/ {print $1, $2, $3, $5, $0}' /var/log/syslog
```
8.  Удаление строк, содержащих "error", и сохранение результата в новый файл
```
...
journalctl
```
2. Просмотр логов конкретной службы:
```
journalctl -u nginx
```
3. Просмотр только ошибок (уровень err):
```
sudo journalctl -u <имя_службы> -p err
```
4. Отслеживание логов в реальном времени:
```
journalctl -f
```
5. Фильтрация логов службы за определённый период: 
```
journalctl -u <имя_службы> --since "2023-10-01" --until "2023-10-02"
или 
sudo journalctl -u <имя_службы> --since "1 hour ago"
```
6. Просмотр логов для процесса с PID = 1234 за последний час:
```
journalctl _PID=1234 --since "1 hour ago"
```
7. Если вы хотите сохранить логи в файл, используйте перенаправление вывода:
```
...
```
4. Включение службы для автозапуска при загрузке:
```
sudo systemctl enable <имя_службы>
```
5. Отключение автозапуска:
```
...
```
7. Просмотр всех служб:
```
systemctl list-unit-files --type=service
```


# "Компьютерная архитектура" и "Аппаратное обеспечение" 


   1.  Файловые системы Windows:
    
    NTFS (New Technology File System): Основная файловая система Windows, поддерживающая большие файлы, шифрование, права доступа и журналирование.
    
    FAT32: Более старая система, совместимая с большинством устройств, но ограниченная размером файлов до 4 ГБ.
    
    exFAT: Улучшенная версия FAT32, поддерживающая большие файлы и совместимость с разными ОС.
    
    2. Файловые системы Linux:
    EXT (Extended File System): Основная файловая система Linux. Включает версии EXT2, EXT3 (с журналированием) и EXT4 (с улучшенной производительностью и поддержкой больших файлов).
    
    XFS: Высокопроизводительная файловая система, подходящая для работы с большими объемами данных.
    
    Btrfs (B-tree File System): Современная файловая система с функциями моментальных снимков и самовосстановления.
    3. Кроссплатформенные файловые системы:
    
    exFAT: Поддерживается Windows, macOS и Linux, используется для флеш-накопителей и SD-карт.
    
    ISO 9660: Используется для оптических дисков (CD/DVD), совместима с большинством ОС.
    
    4. Файловые системы macOS:
    
    HFS+ (Hierarchical File System Plus): Ранее основная файловая система macOS.
    
    APFS (Apple File System): Современная файловая система macOS, оптимизированная для SSD и с поддержкой шифрования.
    Специализированные файловые системы:
    
    ZFS: Используется в системах виртуализации и серверах, поддерживает моментальные снимки и самовосстановление данных.
    
    CXFS: Расширение XFS для кластерных систем, применяется в видеомонтаже и других высокопроизводительных задачах.
